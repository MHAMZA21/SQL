...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`classicmodel` /*!40100 DEFAULT CHARACTER SET latin1 */;
...
## We want to know product code, product name and msrp of the products whose price of each product is < $100 ----- for this we use two table and using subquery.alter
...
create table players (player_id int, e_names varchar(25),country varchar(25),goals int);


insert into players values
(101,'Sam','USA',6),
(103,'Danial','England',7),
...
## STORED PROCEDURES
## An stored procedure is sql code that you can save so that code can be used over and over again
## In this example I want a stored procedure that will return a list of player who have scored 6 goals in a tournament
...
select * from emp_details group by salary order by salary desc limit 1,1;

## Stored procedure using IN parameter
## create a example to fetch or display the top record of employee based on their salaries 
...
DROP delimiter IF EXISTS `upd_salary`;
delimiter //
create procedure upd_sal(IN temp_name varchar(20), IN new_salary float)
begin 
...
call upd_sal('hamza', 50000);
DROP PROCEDURE [IF EXISTS] upd_salary;

## stored procedure using out paramters
## suppose we wnat to count --- we create total employees as an outer parameter and data type would be integer
select * from employees;
select * from emp_details;
select * from products;

delimiter //
create procedure counts_employees(OUT Total_emp int)
begin
...
call counts_employees(@F_emp);
select @F_emp as female_employees; 


delimiter //
create procedure count_male_employee(OUT total_male_emp int)
begin
select count(name) into total_male_emp from emp_details
where sex = 'M';
end //
delimiter ; 

call count_male_employee(@M_emp);
select @M_emp as male_employees;
